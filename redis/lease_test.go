...
func TestLease_Lock_MultipleWaiters(t *testing.T) {
	...
	for i := range numWaiters {
		...
		// Register the cleanup synchronously: registering it from inside the
		// goroutine could land after the test body has finished, which is not
		// guaranteed to be safe.
		t.Cleanup(func() { _ = leases[i].Unlock(context.Background()) })
		go func() {
